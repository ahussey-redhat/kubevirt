...
const (
	qemuGDBPortAnnotation        = "gdb.vm.kubevirt.io/GDBPort" /* allows user to define a port for GDB */
...
func onDefineDomain(vmiJSON []byte, domainXML []byte) ([]byte, error) {
	...
	if GDBPort, found := annotations[qemuGDBPortAnnotation]; found {
		domainSpec.QEMUCmd.QEMUArg = append(domainSpec.QEMUCmd.QEMUArg, domainSchema.Arg{
			Value: "tcp::" + GDBPort,
		})
	}
	if _, found := annotations[qemuGDBPortAnnotation]; !found {
		domainSpec.QEMUCmd.QEMUArg = append(domainSpec.QEMUCmd.QEMUArg, domainSchema.Arg{
			Value: "tcp::1234",
		})
	}
	...
	log.Log.Info("Successfully updated original domain spec with requested GDB attributes")

	return newDomainXML, nil
}

func main() {
	log.InitializeLogging("gdb-hook-sidecar")

	var version string
	pflag.StringVar(&version, "version", "", "hook version to use")
	pflag.Parse()

	socketPath := filepath.Join(hooks.HookSocketsSharedDirectory, "gdb.sock")
	...
	if version == "" {
		panic(fmt.Errorf("usage: \n        /gdb-hook-sidecar --version v1alpha1|v1alpha2"))
	}
